...
class AuthorityClient:
    ...
    async def search_gnd(
        self, query: str, entity_type: Optional[AuthorityType] = None, limit: int = 10
        ...
        try:
            ...
            for item in data.get("member", []):
                ...
                if "dateOfBirth" in item:
                    birth = (
                        item["dateOfBirth"][0]
                        if isinstance(item["dateOfBirth"], list)
                        else item["dateOfBirth"]
                    )
                if "dateOfDeath" in item:
                    death = (
                        item["dateOfDeath"][0]
                        if isinstance(item["dateOfDeath"], list)
                        else item["dateOfDeath"]
                    )
                ...
                record = AuthorityRecord(
                    ...
                    type=item_type,
                    description=(
                        item.get("biographicalOrHistoricalInformation", [None])[0]
    ...
    async def get_gnd(self, gnd_id: str) -> Optional[AuthorityRecord]:
        ...
        try:
            ...
            return AuthorityRecord(
                ...
                birth_date=(
                    item.get("dateOfBirth", [None])[0]
                    if item.get("dateOfBirth")
                    else None
                ),
                death_date=(
                    item.get("dateOfDeath", [None])[0]
                    if item.get("dateOfDeath")
                    else None
                ),
                alternate_names=item.get("variantName", []),
                related_ids=(
                    {"viaf": item.get("sameAs", [{}])[0].get("id", "")}
                    if item.get("sameAs")
                    else {}
                ),
                raw_data=item,
                    ...
                    if value:
                        ...
                        for ref in claim_data.get("references", []):
                            for snak in ref.get("snaks", {}).get("P248", []):
                                ...
                                if ref_id:
                                    refs.append(
                                        f"https://www.wikidata.org/wiki/{ref_id}"
                                    )
    ...
    async def verify_person_fact(
        self, person_name: str, claim_type: str, claimed_value: Any
    ) -> dict:
        """
        ...
        # In Wikidata suchen
        wikidata_results = await self.search_wikidata(
            person_name, AuthorityType.PERSON, 1
        )
