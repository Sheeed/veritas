...
class AuthoritySource(ABC):
    ...
    ) -> PersonNode | EventNode | LocationNode | OrganizationNode | None:
...
class GNDSource(AuthoritySource):
    ...
    def _parse_gnd_record(
        self, data: dict, entity_type: NodeType
        ...
        if data.get("professionOrOccupation"):
            occupations = [
                o.get("label", "")
                for o in data.get("professionOrOccupation", [])
...
class LOCSource(AuthoritySource):
    ...
    async def _get_loc_detail(
        self, lccn: str, entity_type: NodeType
    ) -> AuthorityRecord | None:
    ...
    def _parse_loc_record(
        self, data: dict, lccn: str, entity_type: NodeType
    ) -> AuthorityRecord | None:
...
class GettySource(AuthoritySource):
    ...
    def _parse_getty_binding(
        ...
        source_type = self.VOCABULARIES.get(
            vocab, ("ulan", AuthoritySourceType.GETTY_ULAN)
        )[1]
...
class AuthoritySourceManager:
    ...
    async def import_to_database(
        ...
        return {
            ...
            "authority_ids": [aid for r in results for aid in r.authority_ids],
        }
