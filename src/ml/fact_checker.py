...
class LRUCache:
    ...
    def stats(self) -> Dict[str, Any]:
        valid = sum(1 for _, (_, ts) in self._cache.items() if datetime.now() - ts < self._ttl)
...
class BM25Index:
    ...
    STEM_SUFFIXES = ['en', 'er', 'es', 'em', 'st', 'end', 'ung', 'heit', 'keit', 'lich', 'isch']
    ...
    def search(self, query: str, top_k: int = 5, min_score: float = 0.5) -> List[Tuple[str, float]]:
        if not self._docs:
            return []
        ...
        if sorted_results:
            ...
            normalized = [(doc_id, score / max_score) for doc_id, score in sorted_results]
            filtered = [(doc_id, score) for doc_id, score in normalized if score >= min_score]
...
class LocalKnowledgeBase:
    ...
    def search(self, claim: str, threshold: float = 0.5, top_k: int = 1) -> Optional[Dict]:
        if not self._facts:
            return None
    ...
    def search_multiple(self, claim: str, top_k: int = 5, threshold: float = 0.3) -> List[SearchResult]:
        ...
        for pattern_name, pattern in patterns.items():
            ...
            if match:
                if pattern_name == "country_continent":
                    return await self._verify_country_continent(match.group(1), match.group(2))
                elif pattern_name == "capital":
                    return await self._verify_capital(match.group(1), match.group(2))
        
        return None
    
    async def _verify_country_continent(self, country: str, continent: str) -> Optional[Dict]:
        sparql = f"""
        ...
        try:
            client = await self._get_client()
            response = await client.get(self.SPARQL_URL, params={"query": sparql, "format": "json"})
            ...
            if results:
                ...
                return {
                    "confirms": confirms,
                    "explanation": f"{country} liegt in {actual}." if actual else "Kontinent nicht gefunden.",
                    "actual_value": actual,
                }
...
class VeritasFactChecker:
    ...
    def __init__(self, storage_path: str = "data/self_learning", cache_size: int = 1000):
    ...
    async def _parallel_check(self, claim: str, local_hint: Optional[Dict] = None) -> FactCheckResult:
        llm_task = self.groq.analyze(claim)
        wd_task = self.wikidata.verify_claim(claim)
        wp_task = self._get_wikipedia_context(claim)
        
        results = await asyncio.gather(llm_task, wd_task, wp_task, return_exceptions=True)
        ...
        return self._merge_verdicts(claim, verdicts, evidence, llm_used, wikidata_used, wikipedia_used)
    ...
    def _merge_verdicts(self, claim: str, verdicts: List[Dict], evidence: List[Evidence],
                        llm_used: bool, wikidata_used: bool, wikipedia_used: bool) -> FactCheckResult:
        if not verdicts:
            return FactCheckResult(
                claim=claim,
    ...
    # v3: Search API
    def search_kb(self, query: str, top_k: int = 5, threshold: float = 0.3) -> List[SearchResult]:
...
if __name__ == "__main__":
    async def test():
        ...
        for claim in claims:
            ...
            print(f"  Sources: LLM={result.llm_used}, WD={result.wikidata_used}, KB={result.local_kb_used}")
