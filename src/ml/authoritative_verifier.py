...
class FactCheckClient:
    ...
    async def search_google_factcheck(self, query: str) -> AuthoritativeSourceResult:
        ...
        try:
            ...
            claim = claims[0]
            claim_review = (
                claim.get("claimReview", [{}])[0] if claim.get("claimReview") else {}
            )
    ...
    async def search_crossref(self, query: str) -> AuthoritativeSourceResult:
        ...
        try:
            response = await self.client.get(
                ...
                params=params,
                headers={
                    "User-Agent": "HistoryGuardian/1.0 (mailto:contact@example.com)"
                },
            )
            response.raise_for_status()
            data = response.json()
            ...
            author_str = (
                ", ".join(
                    [f"{a.get('given', '')} {a.get('family', '')}" for a in authors[:3]]
                )
                if authors
                else "Unknown"
            )
            ...
            return AuthoritativeSourceResult(
                ...
                relevance_score=best.get("score", 0)
                / 100,  # CrossRef score normalisieren
                reliability_score=TIER_RELIABILITY[SourceTier.TIER_3_ACADEMIC],
                raw_data=best,
            )
    ...
    async def search_open_library(self, query: str) -> AuthoritativeSourceResult:
        ...
        try:
            ...
            return AuthoritativeSourceResult(
                ...
                url=(
                    f"https://openlibrary.org{best.get('key', '')}"
                    if best.get("key")
                    else None
                ),
...
class AuthoritativeSourceVerifier:
    ...
    def _aggregate_results(
        self, claim: str, results: List[AuthoritativeSourceResult]
    ) -> AuthoritativeVerificationResult:
        ...
        # Höchste Tier finden
        tier_order = [
            SourceTier.TIER_1_AUTHORITY,
            SourceTier.TIER_2_FACTCHECK,
            SourceTier.TIER_3_ACADEMIC,
            SourceTier.TIER_4_ENCYCLOPEDIA,
        ]
        highest_tier = None
        ...
        weighted_relevance = (
            sum(r.relevance_score * r.reliability_score for r in found_results)
            / total_weight
            if total_weight > 0
            else 0
        )
        ...
        fact_check_results = [
            r for r in found_results if r.source_type == "fact_check" and r.rating
        ]
        ...
        if highest_tier:
            summary_parts.append(
                f"Highest tier: {tier_names.get(highest_tier, highest_tier.value)}."
            )
        ...
        if confidence >= 0.8:
            ...
            recommendation = (
                "Limited authoritative support. Additional verification recommended."
            )
        else:
            recommendation = (
                "Minimal authoritative support found. Treat this claim with caution."
            )
