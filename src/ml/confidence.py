...
from src.validation.validator import FullValidationResult, ValidationIssue, IssueSeverity
...
class FeatureExtractor:
    ...
    def extract_from_extraction(
        ...
        features.num_persons = len([n for n in extraction.nodes if n.node_type == NodeType.PERSON])
        features.num_events = len([n for n in extraction.nodes if n.node_type == NodeType.EVENT])
        features.num_locations = len([n for n in extraction.nodes if n.node_type == NodeType.LOCATION])
        features.num_dates = len([n for n in extraction.nodes if n.node_type == NodeType.DATE])
        features.num_organizations = len([n for n in extraction.nodes if n.node_type == NodeType.ORGANIZATION])
        
        # Vollständigkeit
        if features.num_nodes > 0:
            features.relationships_per_node = features.num_relationships / features.num_nodes
            
            # Nodes mit Daten
            nodes_with_dates = 0
            for node in extraction.nodes:
                if isinstance(node, PersonNode) and (node.birth_date or node.death_date):
                    nodes_with_dates += 1
            ...
            features.avg_description_length = sum(desc_lengths) / len(desc_lengths) if desc_lengths else 0
    ...
    def enrich_with_validation(
        ...
        if validation_result.entity_matches:
            features.entity_match_rate = len(validation_result.entity_matches) / max(1, features.num_nodes)
        ...
        features.num_critical_issues = len([
            i for i in validation_result.all_issues 
    ...
    def to_vector(self, features: ClaimFeatures) -> np.ndarray:
        ...
        return np.array([
            ...
            features.date_span_days / 365 if features.date_span_days else 0.0,  # In Jahren
...
class EnsembleConfidenceScorer:
    ...
    def score_extraction(
        ...
        features = self.feature_extractor.extract_from_extraction(extraction, source_text)
        
        if validation_result:
            features = self.feature_extractor.enrich_with_validation(features, validation_result)
    ...
    def add_training_example(
        ...
        self._training_examples.append(TrainingExample(
            features=features,
    ...
    def train(self, min_examples: int = 50) -> dict[str, Any]:
        ...
        y = np.array([ex.true_label for ex in self._training_examples], dtype=np.float32)
        ...
        accuracy = sum(p == ex.true_label for p, ex in zip(predictions, self._training_examples))
    ...
    def load(self, path: str) -> None:
        ...
        logger.info(f"Model loaded from {path} (trained on {state['n_examples']} examples)")
