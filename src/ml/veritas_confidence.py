...
class VeritasFeatureExtractor:
    ...
    def extract(
        ...
        features.has_dates = any(
            re.search(p, text, re.IGNORECASE) for p in self.DATE_PATTERNS
        )
        ...
        features.uses_absolute_language = any(
            term in text_lower for term in self.ABSOLUTE_TERMS
        )
        features.has_citations = bool(
            re.search(r"\[\d+\]|\(\d{4}\)|according to|laut|nach", text_lower)
        )
        ...
        features.mentions_time_period = any(
            ind in text_lower for ind in time_indicators
        )

        location_indicators = ["in", "at", "from", "near", "city", "country", "region"]
        features.mentions_location = any(
            ind in text_lower for ind in location_indicators
        )
        ...
        if sources:
            ...
            features.has_primary_source = any(
                s.get("type") == "primary" for s in sources
            )
            features.has_academic_source = any(
                s.get("type") == "academic" for s in sources
            )
            features.has_authority_source = any(
                s.get("type") == "authority" for s in sources
            )

        # === Validation Features ===
        if validation_result:
            features.entity_verification_score = validation_result.get(
                "entity_score", 0.0
            )
            features.date_consistency_score = validation_result.get("date_score", 0.0)
            features.narrative_pattern_match = validation_result.get(
                "narrative_score", 0.0
            )
