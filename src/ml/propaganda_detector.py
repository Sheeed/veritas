...
class PropagandaDetector:
    ...
    def analyze(self, text: str) -> PropagandaAnalysis:
            ...
            for pattern in lang_pattern.patterns:
                try:
                    ...
                    if matches:
                        ...
                        confidence = (
                            min(1.0, 0.5 + 0.1 * len(matches)) * lang_pattern.weight
                        )
                        ...
                        existing = [
                            d
                            for d in detected_techniques
                            if d.technique == lang_pattern.technique
                        ]
        ...
        if detected_techniques:
            raw_score = sum(d.confidence for d in detected_techniques) / len(
                detected_techniques
            )
