...
class ResilientClient:
    ...
    def __init__(self, timeout: float = 60.0, max_retries: int = 3, backoff_factor: float = 1.0):
    ...
    async def get_with_retry(self, url: str, params: Optional[Dict] = None) -> Tuple[Optional[httpx.Response], Optional[str]]:
        last_error = None
...
class WikidataFactMiner:
    ...
    async def _query(self, sparql: str, query_name: str = "query") -> Tuple[List[Dict], Dict]:
    ...
    async def mine_country_facts(self, limit: int = 100, offset: int = 0) -> Tuple[List[VerifiedFact], Dict]:
        sparql = f"""
        ...
        for r in results:
            ...
            if not country or not continent or country.startswith("Q") or continent.startswith("Q"):
                continue
            
            fact_id = f"geo_{hashlib.md5(f'{country}_{continent}'.encode()).hexdigest()[:8]}"
    ...
    async def mine_capital_facts(self, limit: int = 100, offset: int = 0) -> Tuple[List[VerifiedFact], Dict]:
        sparql = f"""
        ...
        for r in results:
            ...
            if not country or not capital or country.startswith("Q") or capital.startswith("Q"):
                continue
            
            fact_id = f"cap_{hashlib.md5(f'{country}_{capital}'.encode()).hexdigest()[:8]}"
            facts.append(VerifiedFact(
                id=fact_id,
    ...
    async def mine_person_facts(self, limit: int = 100, offset: int = 0) -> Tuple[List[VerifiedFact], Dict]:
        sparql = f"""
        ...
        for r in results:
            ...
            fact_id = f"bio_{hashlib.md5(f'{person}_{birth_year}'.encode()).hexdigest()[:8]}"
            facts.append(VerifiedFact(
                id=fact_id,
    ...
    async def mine_all_parallel(self, batch_size: int = 100, offsets: Dict[str, int] = None) -> Dict[str, Any]:
        """v3: Paralleles Mining aller Faktentypen (4x schneller)."""
        offsets = offsets or {}
        
        tasks = [
            self.mine_country_facts(limit=batch_size, offset=offsets.get("countries", 0)),
            self.mine_capital_facts(limit=batch_size, offset=offsets.get("capitals", 0)),
...
class MisconceptionsMiner:
    MISCONCEPTIONS = [
        {"claim": "Napoleon war sehr klein", "truth": "Napoleon war mit 1,69m durchschnittlich gross fuer seine Zeit", "category": "biographical"},
        {"claim": "Die Chinesische Mauer ist vom Mond aus sichtbar", "truth": "Die Chinesische Mauer ist vom Mond aus nicht sichtbar", "category": "geographic"},
        {"claim": "Einstein war schlecht in Mathe", "truth": "Einstein war ein ausgezeichneter Mathematikstudent", "category": "biographical"},
        {"claim": "Kolumbus wollte beweisen dass die Erde rund ist", "truth": "Im 15. Jahrhundert war bereits bekannt dass die Erde rund ist", "category": "historical"},
        {"claim": "Marie Antoinette sagte Sollen sie Kuchen essen", "truth": "Es gibt keinen Beweis dass Marie Antoinette dies je sagte", "category": "quote"},
        {"claim": "Wikinger trugen gehoernte Helme", "truth": "Es gibt keine historischen Belege fuer gehoernte Wikingerhelme", "category": "historical"},
        {"claim": "Menschen im Mittelalter dachten die Erde sei flach", "truth": "Gebildete Menschen im Mittelalter wussten dass die Erde rund ist", "category": "historical"},
        {"claim": "Der Hundertjaehrige Krieg dauerte 100 Jahre", "truth": "Der Hundertjaehrige Krieg dauerte 116 Jahre (1337-1453)", "category": "historical"},
    ]
...
class AdversarialGenerator:
    ...
    def generate_false_variants(self, true_facts: List[VerifiedFact], max_variants: int = 50) -> List[VerifiedFact]:
        false_facts = []
        ...
        for fact in true_facts[:max_variants]:
            if fact.claim_type == "geographic" and len(fact.entities) >= 2:
                ...
                if continent in self.CONTINENT_SWAPS:
                    ...
                    false_facts.append(VerifiedFact(
                        id=fact_id,
...
class SelfImprovingFactChecker:
    ...
    async def mine_facts(self) -> Dict[str, Any]:
        ...
        BATCH = 100
        stats = {
            "geographic": 0, "capitals": 0, "biographical": 0,
            "misconceptions": 0, "adversarial": 0,
            "total_fetched": 0, "total_new": 0, "total_in_db": len(self._facts),
            "errors": [], "debug": {},
        }
        ...
        # 3. ADVERSARIAL
        true_geo = [f for f in self._facts.values() if f.is_true and f.claim_type == "geographic"]
        ...
        new_geo = [f for f in true_geo if not any(e in used_entities for e in f.entities)][:50]
        ...
        stats["total_new"] = stats["geographic"] + stats["capitals"] + stats["biographical"] + stats["misconceptions"] + stats["adversarial"]
        stats["total_in_db"] = len(self._facts)
        stats["offsets"] = dict(self._offsets)
        stats["mining_time_seconds"] = round((datetime.now() - start_time).total_seconds(), 1)
        
        logger.info(f"Mining complete in {stats['mining_time_seconds']}s: +{stats['total_new']} facts")
        return stats
    
    def get_training_data(self, limit: int = 1000, balanced: bool = False) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "id": f.id, "claim": f.claim, "is_true": f.is_true,
                "explanation": f.explanation, "source": f.source.value,
                "claim_type": f.claim_type, "confidence": f.confidence,
            }
            for f in facts
        ]
