...
class AutoLearnSystem:
        ...
        try:
            ...
            for item in data:
                if item.get('extraction_date'):
                    item['extraction_date'] = item['extraction_date'].isoformat() if isinstance(item['extraction_date'], datetime) else item['extraction_date']
                if item.get('reviewed_at'):
                    item['reviewed_at'] = item['reviewed_at'].isoformat() if isinstance(item['reviewed_at'], datetime) else item['reviewed_at']
    ...
    def _extract_keywords(self, text: str) -> List[str]:
        ...
        stopwords = {
            ...
            'should', 'there', 'these', 'those', 'being', 'about',
            'dass', 'eine', 'einen', 'einer', 'einem', 'sind', 'wird',
            'wurde', 'werden', 'haben', 'hatte', 'sein', 'seine', 'seiner',
        }
        
        # Häufige historische Terme behalten
        historical_terms = {
            'war', 'battle', 'king', 'emperor', 'revolution', 'century',
            'krieg', 'schlacht', 'könig', 'kaiser', 'revolution', 'jahrhundert',
        }
    ...
    def _guess_category(self, text: str, claim: Dict) -> MythCategory:
        ...
        # Person indicators
        person_indicators = ['was', 'war', 'geboren', 'born', 'died', 'starb', 'said', 'sagte']
        if any(ind in text_lower for ind in person_indicators):
            # Check if it's a quote
            if '"' in text or "'" in text or 'said' in text_lower or 'sagte' in text_lower:
                return MythCategory.QUOTE_MYTH
            return MythCategory.PERSON_MYTH
        
        # War indicators
        war_indicators = ['war', 'battle', 'krieg', 'schlacht', 'soldier', 'soldat', 'army', 'armee']
        if any(ind in text_lower for ind in war_indicators):
            return MythCategory.WAR_MYTH
        
        # Event indicators
        event_indicators = ['happened', 'occurred', 'event', 'ereignis', 'discovery', 'entdeckung']
        if any(ind in text_lower for ind in event_indicators):
            return MythCategory.EVENT_MYTH
        
        # Origin indicators
        origin_indicators = ['invented', 'erfunden', 'origin', 'ursprung', 'first', 'erste']
    ...
    def _guess_era(self, text: str, claim: Dict) -> HistoricalEra:
        ...
        # Ancient
        ancient = ['ancient', 'antik', 'roman', 'römisch', 'greek', 'griechisch', 'bc', 'v.chr', 'caesar', 'cleopatra']
        if any(term in text_lower for term in ancient):
            return HistoricalEra.ANCIENT
        
        # Medieval
        medieval = ['medieval', 'mittelalter', 'knight', 'ritter', 'crusade', 'kreuzzug', 'castle', 'burg']
        if any(term in text_lower for term in medieval):
            return HistoricalEra.MEDIEVAL
        
        # Early Modern
        early_modern = ['renaissance', '1500', '1600', 'columbus', 'kolumbus', 'reformation']
        if any(term in text_lower for term in early_modern):
            return HistoricalEra.EARLY_MODERN
        
        # Modern
        modern = ['1700', '1800', '1900', 'napoleon', 'world war', 'weltkrieg', 'industrial', 'industriell']
        if any(term in text_lower for term in modern):
            return HistoricalEra.MODERN
        
        # Contemporary
        contemporary = ['2000', '21st century', '21. jahrhundert', 'internet', 'digital']
