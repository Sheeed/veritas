...
class ExtractionAgentV2:
    ...
    async def _extract_with_consistency(
        ...
        for i in range(self.num_extractions):
            ...
            tasks.append(
                self._single_extraction_with_temp(text, use_cot, use_few_shot, temp)
            )
        ...
        valid_extractions = [
            e for e in extractions if isinstance(e, KnowledgeGraphExtraction)
        ]
    ...
    def _build_consensus(
        ...
        consensus_nodes = [
            node for key, node in node_data.items() if node_counts[key] >= min_count
        ]

        consensus_relationships = [
            rel for key, rel in rel_data.items() if rel_counts[key] >= min_count
        ]
...
class ExtractionEvaluator:
    ...
    def calculate_metrics(
        ...
        true_nodes = {
            f"{n.node_type.value}:{n.name.lower()}" for n in ground_truth.nodes
        }
