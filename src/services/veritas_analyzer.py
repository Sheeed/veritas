...
class VeritasAnalyzer:
    ...
    def __init__(
        ...
        logger.info(f"Authority Sources: {'aktiviert' if self.use_authority_sources else 'deaktiviert'}")
    ...
    async def analyze(
        ...
        if myth_match:
            ...
            return self._create_analysis_from_myth(analysis_id, text, myth_match, language)
        ...
        if not claims_list:
            claims_list = [{"claim": text, "entities": [], "dates": [], "locations": []}]
        ...
        for claim_data in claims_list:
            ...
            if claim_myth:
                claim_analysis = self._create_claim_from_myth(claim_myth, claim_text, language)
            ...
            if verify_with_sources and self.authority_client and entities:
                source_info, authority_sources = await self._gather_authority_info(entities)
            ...
            claim_analysis = ClaimAnalysis(
                ...
                context_status=ContextStatus(context_result.get("context_status", "complete")) if context_result else ContextStatus.COMPLETE,
                narrative_status=NarrativeStatus(narrative_result.get("narrative_status", "neutral")) if narrative_result else NarrativeStatus.NEUTRAL,
                confidence=self._score_to_level(fact_result.get("confidence_score", 0.5)),
                ...
                what_is_missing=context_result.get("important_omissions", []) if context_result else [],
                narrative_match=self._build_narrative_match(narrative_result) if narrative_result else None,
    ...
    async def _gather_authority_info(
        ...
        for entity in entities[:3]:  # Max 3 Entities
            try:
                ...
                for record in results.get("wikidata", []):
                    if record.id:
                        facts = await self.authority_client.get_wikidata_facts(record.id)
                        for fact in facts[:5]:
        ...
        return "\n".join(info_parts) if info_parts else "Keine autoritativen Daten gefunden.", sources
    ...
    async def _analyze_context(self, claim: str, fact_result: dict) -> dict:
        ...
        prompt = build_context_analysis_prompt(
            claim, 
            fact_result.get("fact_status", "unknown")
        )
    ...
    def _detect_language(self, text: str) -> str:
        """Erkennt Sprache."""
        german_indicators = ["der", "die", "das", "und", "ist", "war", "ä", "ö", "ü", "ß"]
    ...
    def _match_myth(self, claim: str) -> Optional[HistoricalMyth]:
        ...
        for myth in self.myths_db.myths.values():
            ...
            if myth.claim_en and (myth.claim_en.lower() in claim_lower or claim_lower in myth.claim_en.lower()):
                score += 10
            
            # Keyword matches
            keyword_matches = sum(
                1 for kw in myth.keywords 
            ...
            if myth.claim_en:
                similarity_en = self._text_similarity(claim_lower, myth.claim_en.lower())
    ...
    def _create_claim_from_myth(
        ...
        status_str = _get_enum_value(myth.status)
        if status_str in ["false", "myth"]:
            what_is_true = [myth.truth if language == "de" else (myth.truth_en or myth.truth)]
            what_is_false = [myth.claim if language == "de" else (myth.claim_en or myth.claim)]
        else:
    ...
    def _build_narrative_match(self, narrative_result: dict) -> Optional[NarrativeMatch]:
        ...
        return NarrativeMatch(
            ...
            is_known_propaganda=narrative_result.get("narrative_status") == "propaganda",
