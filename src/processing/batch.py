...
class BatchJobConfig(BaseModel):
    ...
    max_concurrent: int = Field(default=5, ge=1, le=20, description="Max parallele Verarbeitungen")
    max_retries: int = Field(default=3, ge=0, le=10, description="Max Wiederholungen bei Fehler")
    retry_delay_seconds: float = Field(default=1.0, ge=0.1, description="Wartezeit zwischen Retries")
    timeout_seconds: float = Field(default=60.0, ge=10.0, description="Timeout pro Item")
    rate_limit_per_minute: int = Field(default=60, ge=1, description="Max Requests pro Minute")
    ...
    as_facts: bool = Field(default=False, description="Als Facts statt Claims speichern")
...
class BatchJob:
    ...
    def progress(self) -> BatchJobProgress:
        ...
        processed = sum(1 for i in self.items if i.status in [ItemStatus.SUCCESS, ItemStatus.FAILED, ItemStatus.SKIPPED])
...
class BatchProcessor:
        ...
        async with semaphore:
            ...
            for attempt in range(config.max_retries + 1):
                try:
                    ...
                    logger.warning(f"Item {item.id} failed (attempt {attempt + 1}): {e}")
            ...
            item.processing_time_ms = (datetime.now() - start_time).total_seconds() * 1000
    ...
    async def process_job(self, job: BatchJob) -> BatchJobResult:
        ...
        total_nodes = sum(
            len(i.extraction.nodes) 
            for i in job.items 
            if i.extraction
        )
        total_rels = sum(
            len(i.extraction.relationships) 
            for i in job.items 
            if i.extraction
        )
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in f:
                if line.strip():
                    ...
                    source_id = obj.get(id_field) if isinstance(obj, dict) and id_field else None
                    if text.strip():
                        job.add_text(text, source_id=source_id)
